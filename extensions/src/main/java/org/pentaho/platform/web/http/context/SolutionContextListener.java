...
public class SolutionContextListener implements ServletContextListener {
	...
	public void contextInitialized(final ServletContextEvent event) {
		...
		logger.info(Messages.getInstance().getString("SolutionContextListener.INFO_CONTEXT_PATH", //$NON-NLS-1$
		...
		/*
		 * Copy out all the initParameter values from the servlet context and put them
		...
		serverStatusProvider.setStatusMessages(new String[] {
				"Caution, the system is initializing. Do not shut down or restart the system at this time." });
	...
	private void updateStatusMessages(boolean initOk) {
		if (initOk) {
			...
			serverStatusProvider.setStatusMessages(
					new String[] { "Warning, one or more errors occurred during the initialization process." });
		}
	}

	/**
	 * Provide a simple extension point for someone to be able to override the
	 * behavior of the WebApplicationContext. To extend or change behavior, you will
	 * need to extend WebApplicationContext, and extend SolutionContextListener to
	 * override the createWebApplicationContext method. The subclassing is currently
	 * required because the initialization code above makes a specific setProperties
	 * call on the returned ApplicationContext method by casting it to a
	 * WebApplicationContext.
	 * <p/>
	 * Tangible example where this would be needed - context.getRealPath("") doesn't
	 * work the same way on all platforms. In some cases, you need to pass in a
	 * null, not an empty string. For other servers that don't unpack the war, the
	...
	private void setObjectFactory(final ServletContext context) {
		...
		try {
			...
			// Cannot proceed without an object factory, so we'll put some context around
			// what
	...
	/**
	 * Look for a parameter called "pentaho-system-cfg". If found, use its value to
	 * set the the value of the System property "SYSTEM_CFG_PATH_KEY". This value is
	 * used by a LiberatedSystemSettings class to determine the location of the
	 * system configuration file. This is typically pentaho.xml.
	 *
	 * @param context
	 *            ServletContext
	 */
	...
	public void showInitializationMessage(final boolean initOk, final String fullyQualifiedServerUrl) {
		if (PentahoSystem.getObjectFactory().objectDefined(IVersionHelper.class.getSimpleName())) {
			...
			if (initOk) {
				System.out.println(Messages.getInstance().getString("SolutionContextListener.INFO_SYSTEM_READY",
				...
				System.err.println(Messages.getInstance().getString("SolutionContextListener.INFO_SYSTEM_NOT_READY",
						"(" + helper.getVersionInformation(PentahoSystem.class) + ")", fullyQualifiedServerUrl,
						SolutionContextListener.solutionPath)); // $NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			}
		}
	...
	public void contextDestroyed(final ServletContextEvent event) {
		...
		// log everything that goes on here
		logger.info(Messages.getInstance().getString("SolutionContextListener.INFO_SYSTEM_EXITING")); //$NON-NLS-1$
	}
	...
	private String getServerParameter(String paramName, boolean suppressWarning) {
		...
		if (result == null) {
			...
			if (!suppressWarning) {
				logger.warn(Messages.getInstance().getString("SolutionContextListener.WARN_WEB_XML_PARAM_DEPRECATED", //$NON-NLS-1$
						paramName, result));
			}
		}
		return result;
