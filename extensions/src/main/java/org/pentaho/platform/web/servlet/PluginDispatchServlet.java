...
/**
 * Dispatches requests to Servlets provided by BIServer plugins. To define a
 * Servlet in a plugin, simply add a bean element for the Servlet class in the
 * plugin.spring.xml file in your plugin root directory.
 *
...
public class PluginDispatchServlet implements Servlet {
	...
	public void service(final ServletRequest req, final ServletResponse res) throws ServletException, IOException {
		...
		if (!(req instanceof HttpServletRequest)) {
			throw new IllegalArgumentException(
					PluginDispatchServlet.class.getSimpleName() + " cannot handle non HTTP requests"); //$NON-NLS-1$
		}
	...
	protected Servlet getTargetServlet(HttpServletRequest request, HttpServletResponse response)
		...
		do {
			...
			if (targetServlet != null) {
				logger.debug(
						"servlet " + targetServlet.getClass().getName() + " will service request for \"" + dispatchKey
						// $NON-NLS-1$//$NON-NLS-2$
								+ "\""); //$NON-NLS-1$
	...
	/**
	 * Returns the dispatch key for this request. This name is the part of the
	 * request path beyond the servlet base path. I.e. if the PluginDispatchServlet
	 * is mapped to the "/plugin" context in web.xml, then this method will return
	 * "myPlugin/myServlet" given a request to
	...
	/**
	 * Restore the caching once the Plugin Type Tracking system is in place, for now
	 * we'll look-up every time
	 **/
	private synchronized void doInit() throws ServletException {
		...
		for (Map.Entry<String, ListableBeanFactory> pluginBeanFactoryEntry : pluginBeanFactoryMap.entrySet()) {
			...
			Map<String, Servlet> beans = BeanFactoryUtils
			...
			for (Map.Entry<String, Servlet> beanEntry : beans.entrySet()) {
				...
				if (logger.isDebugEnabled()) {
					logger.debug("calling init on servlet " + pluginServlet.getClass().getName() + " serving context "
							+ context); // $NON-NLS-1$//$NON-NLS-2$
				}
				try {
					pluginServlet.init(servletConfig);
				} catch (Throwable t) {
					logger.error("Could not load servlet '" + context + "'", t); //$NON-NLS-1$ //$NON-NLS-2$
																					// //$NON-NLS-3$
				}
			}
		}
