...
/**
 * The purpose of this Java class is to startup a HSQLDB databases. This class
 * should not installed in production environments. This is for samples /
 * demoing only.
 * 
...
public class HsqlDatabaseStarterBean {
		...
		protected boolean checkPort() {
				...
				try {
						...
						if (port == failoverPort) {
								...
								if (allowFailoverToDefaultPort) {
										logger.error(Messages.getErrorString("HsqlDatabaseStarterBean.ERROR_0005_SPECIFIED_PORT_IN_USE", Integer.toString(port), //$NON-NLS-1$
										                "" + failoverPort));
										port = failoverPort;
										...
										return logFailure("HsqlDatabaseStarterBean.ERROR_0008_SPECIFIED_PORT_IN_USE_NO_FAILOVER", Integer.toString(port)); //$NON-NLS-1$
								}
						}
				}
		...
		protected ArrayList<String> getStartupArguments() {
				...
				for (Map.Entry<String, String> entry : databases.entrySet()) {
						...
						logger.debug(MessageFormat.format("Hsqldb database {0} configured to start with name {1}", entry.getValue(), entry.getKey())); //$NON-NLS-1$
						idx++;
				}
