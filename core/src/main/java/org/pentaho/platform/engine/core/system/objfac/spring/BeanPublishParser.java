...
/**
 * Parses the publish tag of a bean. Exposing the bean to the PentahoSystem as
 * an implementation of the given type. Beans can be published by a given type,
 * as all implemented interfaces, as all inherited classes, a combination of all
 * classes and interfaces, or by default as the class of the bean itself.
 * <p/>
 * Attributes embedded in the publish tag become available to the PentahoSystem
 * to allow for querying of registered implementations. An attribute of
 * "priority" is used to determine the order of registered implementations.
 * <p/>
 * <pen:bean class="com.foo.Clazz"> <pen:publish as-type="[ INTERFACES | CLASSES
 * | ALL | Classname ]> <pen:attributes> <pen:attr key="priority" value="50"/>
 * </pen:attributes> </pen:publish> </pen:bean>
 * <p/>
...
public class BeanPublishParser implements BeanDefinitionDecorator {
		...
		public BeanDefinitionHolder decorate(Node node, BeanDefinitionHolder beanDefinitionHolder, ParserContext parserContext) {
				...
				// If this is a republish of a pen:bean, the class will be a FactoryBean and the
				// actual class returned back the
						...
						if (stripNamespace(n.getNodeName()).equals(Const.ATTRIBUTES)) {
								...
								for (int y = 0; y < attrnodes.getLength(); y++) {
										...
										if (stripNamespace(an.getNodeName()).equals(Const.ATTR)) {
												beanDefinitionHolder.getBeanDefinition().setAttribute(an.getAttributes().getNamedItem(Const.KEY).getNodeValue(),
				...
				try {
						...
						if (parserContext.getRegistry().containsBeanDefinition(Const.FACTORY_MARKER) == false) {
								...
								parserContext.getRegistry().registerBeanDefinition(Const.FACTORY_MARKER, BeanDefinitionBuilder.genericBeanDefinition(Marker.class)
								                .setScope(BeanDefinition.SCOPE_PROTOTYPE).addConstructorArgValue(beanFactoryId).getBeanDefinition());
						} else {
								beanFactoryId = (String) parserContext.getRegistry().getBeanDefinition(Const.FACTORY_MARKER).getConstructorArgumentValues()
						...
						throw new RuntimeException("Cannot find class for publish type: " + publishType + " specified on publish of bean id: " + beanDefinitionHolder.getBeanName(),
						                e);
				}
